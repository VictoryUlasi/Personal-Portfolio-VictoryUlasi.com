...
const projects = [
  ...
  },
  {
    ...
    images: [
      ...
      "images/project_Images/Thrust-stand/plot_img.webp",
      "images/project_Images/Thrust-stand/IMG_0943.webp",
      "images/project_Images/Thrust-stand/code_snippet.webp",
    ...
    tools:
      "STM32F446RE, STM32CubeIDE, Embedded-C, MATLAB, HX711, ACS758, UART, SPI, 3D Printing, Soldering, Physics 2, Pulse-Width-Modulation",
    description:
      "A real-time motor performance measurement system built around the STM32F446RE (NUCLEO-F446RE), directly inspired by the drone build and a desire to actually quantify what the motors are capable of. Measures thrust (g), current draw (A), and battery voltage (V) simultaneously and streams live data over UART at 115200 baud as CSV to MATLAB for analysis and plotting. Thrust is measured via a 10kg cantilever beam load cell with an HX711 24-bit ADC; current via an ACS758 50A linear current sensor; and voltage via a 10kΩ/3kΩ resistor divider scaled to the STM32's 3.3V ADC range. Throttle is controlled by a time-based PWM sweep generated directly by the STM32, ramping from 1000µs to 1800µs over 15 seconds. The frame is built on 2020 aluminum extrusion with 3D printed motor and electronics mounts.",
    challenges:
      "The biggest integration challenge was getting all three sensors running simultaneously without any of them blocking each other. The HX711 uses a clocked serial protocol, the voltage and current sensors use analog ADC reads, and all of it had to stream out over UART without stalling the loop. The HX711 runs at 10Hz, meaning most loop iterations have no new data ready. Getting that timing wrong caused the firmware to freeze waiting for the sensor, which in turn caused the ESC to lose signal and cut the motor mid-sweep. The fix was a 50ms timeout on the HX711 read that returns the last known good value instead of blocking. Each sensor had its own calibration challenges. The HX711 needed a scale factor of 104.26 counts per gram, calibrated with known weights, and tares automatically on startup. The ACS758 picked up enough electrical noise from the motor to need an 8-sample moving average filter in firmware. The voltage divider had to be scaled precisely to bring the 3S LiPo's 12.6V maximum down to 3.3V for the ADC, with the firmware reversing that ratio to reconstruct actual pack voltage. Determining the ESC's real PWM range also took iteration: the standard 1000-2000µs range caused the ESC to trigger protection cutoffs at high throttle, and through testing the working range turned out to be 1000-1800µs.",
    results:
      "All three sensors read cleanly across the full throttle sweep with no cutoffs. Peak thrust reached 1391g at full throttle, with voltage sagging from 12.0V to 11.3V under load and current draw hitting the 20A readable limit of the sensor. The MATLAB script logs timestamped CSV, plots all four channels in real time, and computes specific thrust in g/W across the sweep. The characterization data will feed directly into motor selection and power budgeting for planned fixed-wing and utility drone builds. Next step naturally would be to design and implement a custom PCB to replace the breadboard as the wiring is really messy.",
    video: "",
    youtube: "",
  ...
  },
  {
    ...
    challenges:
      "The biggest challenge was translating statics into code. Setting up equilibrium as Ax = b meant understanding what each matrix coefficient actually represented physically, not just plugging into a formula. Distributed loads added real complexity: the equivalent point load works fine for reactions but falls apart for the shear diagram, where load intensity has to be evaluated continuously across the beam using linear interpolation and running area calculations at every point. Keeping sign conventions consistent across point loads, distributed loads, and reactions without subtle errors required more careful thought than expected. A smaller but real challenge was correctly locating user-entered positions on a fixed array of computed points, since the array cannot hit every possible decimal value, there is always a small gap between where the user says a load is and where the code actually evaluates it.",
    results:
