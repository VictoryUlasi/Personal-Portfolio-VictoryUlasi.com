...
const projects = [
  ...
  },
  {
    ...
    challenges:
      "The biggest integration challenge was getting three different sensing modalities to run at the same time without any of them blocking each other — a clocked serial protocol for the HX711 load cell, analog ADC reads for voltage and current, and UART output for streaming data. Coordinating all of that required careful attention to HAL timing in STM32CubeIDE. Each sensor had its own calibration and quirks. The HX711 needed a scale factor of 104.26 counts per gram, calibrated using known weights. Tare runs automatically on startup, which means any load sitting on the mount at power-on throws off every reading afterward. The ACS758 current sensor picked up enough noise to need filtering in firmware. The voltage divider had to be scaled precisely — a 10kΩ over 3kΩ resistor network brings the 3S LiPo's 12.6V maximum down to the 3.3V the STM32's ADC can handle, and the firmware reverses that ratio to reconstruct the actual pack voltage. Getting all three channels reading stably and streaming clean CSV over UART at the same time was where most of the real engineering happened.",
    results:
      "Hardware is fully assembled and all three sensors are reading cleanly. The load cell is calibrated to sub-gram noise at idle, the voltage divider tracks real-time LiPo sag, and the ACS758 current sensor is validated against known loads. The STM32 streams timestamped CSV over UART at 115200 baud with a moving average filter on current for noise rejection. A MATLAB script reads the serial port, logs to CSV, and plots thrust, voltage, current, and specific thrust (g/W) across the full throttle sweep. Peak measurements on a partial sweep reached 428g thrust and 20A current draw with visible voltage sag from 11.5V to 10.6V under load, consistent with the A2212 datasheet. Next steps are a full throttle sweep on a charged LiPo, automated PWM throttle control from the STM32, and a custom PCB to replace the breadboard. The characterization data will feed directly into motor selection and power budgeting for planned fixed-wing and utility drone builds.",
    video: "",
    youtube: "",
