...
const projects = [
  ...
  },
  {
    ...
    challenges:
      "The core integration challenge was getting three different sensing modalities to run at the same time without any of them blocking each other — a clocked serial protocol for the HX711 load cell, analog ADC reads for voltage and current, and UART output for streaming data. Coordinating all of that required careful attention to HAL timing in STM32CubeIDE. Each sensor had its own calibration and quirks. The HX711 needed a scale factor of 200.9 counts per gram, calibrated using known weights. Tare runs automatically on startup, which means any load sitting on the mount at power-on throws off every reading afterward. The ACS758 current sensor picked up enough noise to need filtering in firmware. The voltage divider had to be scaled precisely — a 10kΩ over 3kΩ resistor network brings the 3S LiPo's 12.6V maximum down to the 3.3V the STM32's ADC can handle, and the firmware reverses that ratio to reconstruct the actual pack voltage. Getting all three channels reading stably and streaming clean CSV over UART at the same time was where most of the real engineering happened.",
    results:
      "Hardware is done. All three sensors are reading cleanly — load cell, voltage, current — and the STM32 is streaming timestamped CSV over UART at 115200 baud, ready for MATLAB. Still need to filter current noise in firmware, finish the MATLAB plotting script for thrust curves, voltage sag, and motor efficiency (g/W), run a full throttle sweep, and design a PCB to get rid of the breadboard. Once it's all working, the data feeds directly into motor selection and power budgeting for the utility drone and fixed wing builds I have planned.",
    video: "",
    youtube: "",
